...
z = tf.concat([tf.one_hot(z_cat, depth = cat_dim), z_con, z_rand],1)
...
loss_c_r = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=cat_real))
loss_c_d = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=z_cat, logits=cat_fake))
...
with tf.Session() as sess:
    ...
    try:
        while not coord.should_stop():
            ...
            cur_epoch =int( l_d_step / num_batch_per_epoch)
            ...
            if cur_epoch> last_epoch:
                ...
                if cur_epoch % save_epoch==0:
                    # save
                    saver.save(sess, os.path.join('./', 'saver'), global_step=l_d_step)
