...
z = tf.concat( [z, tf.expand_dims(target_cval_1, -1), tf.expand_dims(target_cval_2, -1)],z.get_shape().ndims-1)

z = tf.concat( [z, tf.random_normal((batch_size, rand_dim))],z.get_shape().ndims-1)
...
# classified image
run_generator(np.arange(10).repeat(10), np.random.uniform(0, 1, batch_size), np.random.uniform(0, 1, batch_size),)
